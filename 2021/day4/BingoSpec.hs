...
main = hspec $ do
  describe "Bingo Tests" $ do
    ...
    it "should test a card to see if it is a Winning candidate" $ do
      ...
      isComplete scoredCardColumnWin `shouldBe` True 
    ...
    it "should find the winning card with a completed row or column" $ do
      ...
      findWinner [scoredCardColumnWin] `shouldBe` scoredCardColumnWin
    ...
    it "should calculate the winning score from the winning card" $ do
      ...
      scoreWinner  scoredCard (0,5) `shouldBe` 0
      scoreWinner scoredCard  (1,5) `shouldBe` 237
      scoreWinner scoredCardColumnWin  (1,5) `shouldBe` 242
